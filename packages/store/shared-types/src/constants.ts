...
export const TYPE_OPTIONS = TYPE_DEFS.map(({ id, key, nameZh, nameEn, color }) => ({ id, key, nameZh, nameEn, color }));
...
export const TYPE_ID_BY_NAME = toMap(TYPE_OPTIONS.flatMap(({ id, key, nameZh, nameEn }) => [[nameZh, id], [nameEn, id], [key, id], [String(id), id]]));
export const TYPE_NAME_BY_ID = toMap(TYPE_OPTIONS.map(({ id, nameZh }) => [id, nameZh]));
export const TYPE_ZH_TO_EN = toMap(TYPE_OPTIONS.map(({ nameZh, nameEn }) => [nameZh, nameEn]));
export const TYPE_COLORS = toMap(TYPE_OPTIONS.map(({ nameZh, color }) => [nameZh, color]));
export const TYPE_COLORS_BY_ID = toMap(TYPE_OPTIONS.map(({ id, color }) => [id, color]));
export const TYPE_BG_RGB = toMap(TYPE_DEFS.map(({ nameZh, rgb }) => [nameZh, rgb]));
export const TYPE_BG_RGB_BY_ID = toMap(TYPE_DEFS.map(({ id, rgb }) => [id, rgb]));
export const TYPE_CHART = toMap(TYPE_DEFS.map(({ nameZh, effectiveness }) => [nameZh, effectiveness]));
export const TYPE_CHART_BY_ID = toMap(TYPE_DEFS.map(({ id, effectiveness }) => [id, effectiveness]));
...
export function typeIdToName(typeId) {
  return TYPE_NAME_BY_ID[typeId] || typeId;
}

export function makeTypeBgColors(alpha = 0.10) {
  return toMap(TYPE_DEFS.map(({ nameZh, rgb }) => [nameZh, `rgba(${rgb},${alpha})`]));
}

export function makeTypeBgColorsById(alpha = 0.10) {
  return toMap(TYPE_DEFS.map(({ id, rgb }) => [id, `rgba(${rgb},${alpha})`]));
}
...
export const STAT_KEYS = STAT_OPTIONS.map(({ key }) => key);
export const STAT_ID_BY_KEY = toMap(STAT_OPTIONS.map(({ id, key }) => [key, id]));
export const STAT_KEY_BY_ID = toMap(STAT_OPTIONS.map(({ id, key }) => [id, key]));
export const STAT_LABELS = toMap(STAT_OPTIONS.map(({ key, label }) => [key, label]));
export const STAT_LABELS_BY_ID = toMap(STAT_OPTIONS.map(({ id, label }) => [id, label]));
export const STAT_LABELS_SHORT = toMap(STAT_OPTIONS.map(({ key, shortLabel }) => [key, shortLabel]));
export const STAT_LABELS_SHORT_BY_ID = toMap(STAT_OPTIONS.map(({ id, shortLabel }) => [id, shortLabel]));
export const STAT_LABELS_COMPACT = toMap(STAT_OPTIONS.map(({ key, compactLabel }) => [key, compactLabel]));
export const STAT_LABELS_COMPACT_BY_ID = toMap(STAT_OPTIONS.map(({ id, compactLabel }) => [id, compactLabel]));
...
export const NATURE_ID_BY_NAME = toMap(NATURES.flatMap(({ id, key, nameZh, nameEn }) => [[nameZh, id], [nameEn, id], [key, id], [String(id), id]]));
export const NATURE_NAME_BY_ID = toMap(NATURES.map(({ id, nameZh }) => [id, nameZh]));
export const NATURE_ZH_TO_EN = toMap(NATURES.map(({ nameZh, nameEn }) => [nameZh, nameEn]));
...
export function natureNameToId(nature) {
  return NATURE_ID_BY_NAME[String(nature || "").trim()];
}

export function natureIdToName(natureId) {
  return NATURE_NAME_BY_ID[natureId] || natureId;
}
...
export const LEARN_METHOD_ID_BY_KEY = toMap(LEARN_METHOD_OPTIONS.map(({ id, key }) => [key, id]));
export const LEARN_METHOD_KEY_BY_ID = toMap(LEARN_METHOD_OPTIONS.map(({ id, key }) => [id, key]));
...
export const CATEGORY_ID_BY_NAME = toMap(CATEGORY_OPTIONS.flatMap(({ id, key, nameZh }) => [[nameZh, id], [key, id], [String(id), id]]));
export const CATEGORY_NAME_BY_ID = toMap(CATEGORY_OPTIONS.map(({ id, nameZh }) => [id, nameZh]));
export const CATEGORY_KEY_BY_ID = toMap(CATEGORY_OPTIONS.map(({ id, key }) => [id, key]));
export const CATEGORY_COLORS = toMap(CATEGORY_OPTIONS.map(({ nameZh, colors }) => [nameZh, colors]));
export const CATEGORY_COLORS_BY_ID = toMap(CATEGORY_OPTIONS.map(({ id, colors }) => [id, colors]));
export const CATEGORY_BG_COLORS = toMap(CATEGORY_OPTIONS.map(({ nameZh, colors }) => [nameZh, colors.bg]));
export const CATEGORY_BG_COLORS_BY_ID = toMap(CATEGORY_OPTIONS.map(({ id, colors }) => [id, colors.bg]));
