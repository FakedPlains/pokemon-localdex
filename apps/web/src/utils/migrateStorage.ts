 ...
 */
function isLegacyId(value: unknown): boolean {
...
function needsMigration(): boolean {
  ...
  const box: PokemonConfig[] = readJSON<PokemonConfig>(BOX_KEY);
  const teams: Team[] = readJSON<Team>(TEAMS_KEY);
  ...
  for (const team of teams) {
    for (const member of (team.members || [])) {
      if (isTeamMemberRef(member)) continue; // 引用 box 的成员不需要单独检查
 ...
 * @returns 是否有变更
 */
async function migrateConfig(config: Partial<PokemonConfig>): Promise<boolean> {
  let changed = false;
...
async function performMigration(): Promise<void> {
  ...
  for (const team of teams) {
    for (const member of (team.members || [])) {
      if (isTeamMemberRef(member)) continue; // 引用型成员跳过
      const changed = await migrateConfig(member as Partial<PokemonConfig>);
      if (changed) totalMigrated++;
    }
  }
