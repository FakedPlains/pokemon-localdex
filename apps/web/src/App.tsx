...
type NavKey = "pokedex" | "abilities" | "moves" | "items" | "teams" | "damage" | "typechart";
...
const SEARCHABLE_PAGES = new Set<string>(["pokedex", "items", "moves", "abilities"]);

/* Pages that show the floating filter panel */
const FILTERABLE_PAGES = new Set<string>(["pokedex", "moves", "abilities"]);

/* Pages that show type filter */
const TYPE_FILTER_PAGES = new Set<string>(["pokedex", "moves"]);

/* Pages that show generation filter */
const GEN_FILTER_PAGES = new Set<string>(["pokedex", "moves", "abilities"]);

/* Pages that show category filter (moves only) */
const CATEGORY_FILTER_PAGES = new Set<string>(["moves"]);

// TypeOption and CategoryOption imported from @pokemon-localdex/store-types/constants

function parseRoute(): NavKey {
...
function parsePokemonIdFromHash(): string | null {
...
export default function App() {
  ...
  const [teamDraft, setTeamDraft] = useState<{
    ...
    name: "",
    format: "singles",
    members: Array.from({ length: 6 }, () => createDraftMember(undefined)),
  });
