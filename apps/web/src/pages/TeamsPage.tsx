...
import { BOX_SORT_OPTIONS, getDisplayedBoxConfigs } from "../components/teams/boxSort";
import BoxCard from "../components/teams/BoxCard";
import BoxListRow from "../components/teams/BoxListRow";
...
export default function TeamsPage(_props: TeamsPageProps) {
  const toast = useToast();
  const [boxConfigs, setBoxConfigs] = useState<PokemonConfig[]>([]);
  const [editingConfig, setEditingConfig] = useState<PokemonConfigEditState | null>(null);
  const [isNewConfig, setIsNewConfig] = useState(false);

  const [teams, setTeams] = useState<Team[]>([]);
  const [editingTeam, setEditingTeam] = useState<(Omit<Partial<Team>, 'members'> & { members: (PokemonConfigEditState & { slot: number })[] }) | null>(null);
  const [isNewTeam, setIsNewTeam] = useState(false);
  const [inlineEditSlot, setInlineEditSlot] = useState<number | null>(null);
  const [inlineEditDraft, setInlineEditDraft] = useState<(PokemonConfigEditState & { slot?: number }) | null>(null);
  ...
  const [activeTab, setActiveTab] = useState<"box" | "teams">("box");
  const [boxViewMode, setBoxViewMode] = useState<"card" | "list">("card");
  const [boxSearch, setBoxSearch] = useState("");
  const [boxSortMode, setBoxSortMode] = useState("current");
  const [pickerSearch, setPickerSearch] = useState("");

  const editorRef = useRef<HTMLDivElement>(null);
  const teamEditorRef = useRef<HTMLDivElement>(null);
  ...
  const prevEditingConfigRef = useRef<Partial<PokemonConfig> | null>(null);
  useEffect(() => {
  ...
  const prevEditingTeamRef = useRef<unknown>(null);
  useEffect(() => {
    const wasEditing = prevEditingTeamRef.current != null;
    const isEditing = editingTeam != null;
    prevEditingTeamRef.current = editingTeam ?? null;
  ...
  // ── 盒子操作 ──
  const handleNewConfig = useCallback(() => { setEditingConfig(createDraftMember(undefined)); setIsNewConfig(true); }, []);
  const handleEditConfig = useCallback((config: PokemonConfigDisplay) => { setEditingConfig({ ...config }); setIsNewConfig(false); }, []);
  ...
  const handleDeleteConfig = useCallback((configId: string) => { if (!window.confirm("确定删除这个配置吗？")) return; deleteBoxConfig(configId); refreshBox(); }, [refreshBox]);
  const handleDuplicateConfig = useCallback((configId: string) => { duplicateBoxConfig(configId); refreshBox(); }, [refreshBox]);
  const handleCancelEdit = useCallback(() => { setEditingConfig(null); setIsNewConfig(false); }, []);
  const handleEditingConfigChange = useCallback((updater: PokemonConfigEditState | ((prev: PokemonConfigEditState) => PokemonConfigEditState)) => { setEditingConfig((prev) => (typeof updater === "function" ? updater(prev ?? createDraftMember(undefined)) : updater)); }, []);
  ...
  const handleEditTeam = useCallback((team: Team) => { setEditingTeam({ ...team, members: resolveTeamMembers(team).map((m) => ({ ...m })) }); setIsNewTeam(false); setInlineEditSlot(null); setInlineEditDraft(null); }, []);
  const handleSaveTeam = useCallback(() => {
    if (!editingTeam) return;
    if (!editingTeam.name?.trim()) { toast.error("请输入队伍名称。"); return; }
    const validMembers = (editingTeam.members || []).filter((m): m is PokemonConfigEditState & { slot: number; pokemonId: string } => !!(m && m.pokemonId));
  ...
  const handleDeleteTeam = useCallback((teamId: string) => { if (!window.confirm("确定删除这支队伍吗？")) return; deleteTeam(teamId); refreshTeams(); }, [refreshTeams]);
  ...
  const handleSelectFromBox = useCallback((slot: number, configId: string) => {
    if (!editingTeam) return;
    ...
    const newMember: PokemonConfigDisplay & { slot: number } = { ...config, slot, configId };
    if (idx >= 0) members[idx] = newMember; else members.push(newMember);
    setEditingTeam({ ...editingTeam, members });
  }, [editingTeam, boxConfigs]);
  const handleRemoveMember = useCallback((slot: number) => {
    if (!editingTeam) return;
  ...
  const handleStartInlineEdit = useCallback((slot: number) => { setInlineEditSlot(slot); setInlineEditDraft({ ...createDraftMember(undefined), slot }); setInlineEditIsNew(true); setInlinePickerSearch(""); }, []);
  const handleEditMember = useCallback((slot: number, member: PokemonConfigEditState) => { setInlineEditSlot(slot); setInlineEditDraft({ ...member }); setInlineEditIsNew(false); setInlinePickerSearch(""); }, []);
  const handleInlineEditDraftChange = useCallback((updater: PokemonConfigEditState | ((prev: PokemonConfigEditState) => PokemonConfigEditState)) => { setInlineEditDraft((prev) => (typeof updater === "function" ? updater(prev ?? { ...createDraftMember(undefined), slot: undefined }) : updater)); }, []);
  ...
  // 渐进式槽位：按顺序紧凑排列已有成员 + 1个空位（最多6个）
    const teamSlots = useMemo<(PokemonConfigEditState & { slot: number } | null)[]>(() => {
    if (!editingTeam) return [];
    const members = editingTeam.members || [];
    // 按 slot 排序后紧凑排列
    const sorted = [...members].sort((a, b) => (a.slot || 0) - (b.slot || 0));
    const result: (PokemonConfigEditState & { slot: number } | null)[] = sorted.map((m) => ({ ...m, slot: sorted.indexOf(m) + 1 }));
    // 如果还没满6个，追加一个空位
    if (result.length < 6) {
      result.push(null);
    }
                  ...
                  <div className="cfg-toolbar">
                    ...
                    <div className="team-edit-fields">
                      ...
                      <CustomSelect
                        ...
                        onChange={(val: string) => setEditingTeam({ ...editingTeam, format: val })}
                      />
                    </div>
