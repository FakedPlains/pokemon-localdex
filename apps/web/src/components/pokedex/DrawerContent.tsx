...
export default function DrawerContent({ detail, detailGeneration, onDetailGenerationChange }: DrawerContentProps) {
  const [tab, setTab] = useState("stats");
  const [imageMode, setImageMode] = useState("official");
  const [detailForm, setDetailForm] = useState<string | number>("default");
  const [learnsetMeta, setLearnsetMeta] = useState<LearnsetMeta | null>(null);
  const [learnsetFormOverride, setLearnsetFormOverride] = useState<string | null>(null);
  ...
  const formToLearnsetMap = useMemo(() => {
    ...
    const usedLearnsetKeys = new Set<string>();
  ...
  const mapFormToLearnsetKey = useCallback((form: ResolvedForm | undefined): string | null => {
    if (!form) return null;
    return formToLearnsetMap.get(form.id) || null;
  }, [formToLearnsetMap]);

  // 当切换 detail 形态时，同时联动 learnset 的 formKey
  const handleFormChange = useCallback((formId: string | number) => {
