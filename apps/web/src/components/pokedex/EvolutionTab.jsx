...
/** 渲染单个宝可梦成员卡片 */
function EvoMember({ evo, detail, currentFormId }) {
  const isCurrent =
 ...
 */
function StageGroupView({ chain, detail, currentFormId }) {
 ...
 */
function TreeNode({ node, detail, isRoot, currentFormId }) {
        ...
        {linear.map((child, i) => {
          ...
          return (
            <div key={`${child.evo.toPokemonId}-${child.evo.toFormId || "d"}-${i}`} className="evo-branch-step">
              ...
              <EvoMember evo={child.evo} detail={detail} currentFormId={currentFormId} />
            </div>
          );
        })}
 ...
 */
function BranchView({ chain, detail, currentFormId }) {
...
/* ─── Evolution Tab (懒加载) ─── */
export default function EvolutionTab({ detail, evolutionChain, loading, currentFormId }) {
  if (loading) {
    return (
