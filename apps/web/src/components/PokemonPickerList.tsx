...
export default function PokemonPickerList({ search = "", onSelect }: PokemonPickerListProps) {
  ...
  const loadPage = useCallback(async (currentOffset: number, query: string, reset = false) => {
    setLoadingMore(true);
    try {
        ...
        <table className="cfg-picker-table">
          ...
          <tbody>
            {visible.map((p) => {
              ...
              const bst = STAT_KEYS.reduce((s, k) => s + getStatValue(p.baseStats, k), 0);
              return (
                <tr key={p.slug || p.id} className="cfg-picker-row" onClick={() => onSelect(p)}>
                  <td className="cfg-td-img">
                    {img && <img src={img} alt={p.nameZh || ""} referrerPolicy="no-referrer" />}
                  </td>
                  ...
                  {STAT_KEYS.map((k) => (
                    <td key={k} className="cfg-td-stat">{getStatValue(p.baseStats, k) || "—"}</td>
                  ))}
