  ...
  app.innerHTML = `
    <section class="view-grid items-layout">
      <div class="panel">
        ...
        <div class="item-list">
          ${moves.map((move) => `
            <button class="list-card secondary media-list-card" data-move="${escapeHtml(move.slug || move.id)}">
              <div class="list-thumb move-thumb">
                ${move.image?.url ? `<img src="${escapeHtml(move.image.url)}" alt="${escapeHtml(move.image.alt || move.nameZh)}" referrerpolicy="no-referrer" loading="lazy" />` : `<span>${escapeHtml(move.type || "招式")}</span>`}
              </div>
