...
export function numberQuery(c: any, key: string): number | undefined {
  const v = c.req.query(key);
  return v ? Number(v) : undefined;
}

export function paginatedJson(c: any, result: unknown, offset: number, limit: number) {
...
export function pokemonListQuery(c: any) {
